...
public class DijkstrasAlgorithm {
    ...
    public static void Dijkstras(String nodeName,int weight){        
        node temp= new node(); 
        ...
        if(j<nodeList.size()){                     
            ...
            Dijkstras(visitedNode.get(visitedNode.size()-1),w);                     
        }                
    }
    public static List BubbleSort(List<node> t){
        int n = t.size();  node temp = new node();  
        for(int i=0; i < n; i++){  
            for(int j=1; j < (n-i); j++){  
                if(t.get(j-1).getweight() > t.get(j).getweight()){                                     
                    temp = t.get(j-1);  t.set(j-1, t.get(j));  t.set(j, temp);
                } }  } 
        return t;
    }        
    public static node getNextMin(){
        List <node> temp=new ArrayList<node>(); temp=nodeList;  temp=BubbleSort(temp);
        node n=new node();
        for(int i=0;i<temp.size();i++){
            if(!visitedNode.contains(temp.get(i).getname())){n=temp.get(i);  break;} }    
        return n;
    }    
    public static int getWeight(String e){
        for(int i=0;i<nodeList.size();i++){
            if(nodeList.get(i).getname().equals(e)){return nodeList.get(i).getweight();}
        }
        return 0;
    }
    public static void setParentAndWeight(String s,String e,int c){
        for(int i=0;i<nodeList.size();i++){
            if(nodeList.get(i).getname().equals(e)){
                nodeList.get(i).setParent(s);  nodeList.get(i).setweight(c);}
        }        
    }
    ...
    public static void main(String args[]){
        ...
        FileReader file;                
        try {
            file= new FileReader("C:\\Users\\sushma r iliger\\Desktop\\Dijkstras_edgelist.json");              
            readJsonFile(file);
